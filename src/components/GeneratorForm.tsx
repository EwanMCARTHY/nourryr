...
import { Sparkles, Store, Users, Calendar, DollarSign, Dumbbell, Flame, CheckCircle2, ChevronRight, Info } from 'lucide-react';
...
export const GeneratorForm: React.FC<GeneratorFormProps> = ({
  ...
  const [budget, setBudget] = useState<number>(55);
  ...
  const totalMeals = numberOfDays * mealsPerDay;
  const costPerMeal = (budget / (totalMeals * numberOfPeople)).toFixed(2);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 pb-10">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-zinc-900 via-zinc-900/90 to-zinc-950 p-4 border border-zinc-800/80 shadow-xl">
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
              <Dumbbell className="w-3 h-3" /> Focus Musculation & Protéines
            </span>
            <h2 className="text-lg font-bold text-white tracking-tight">
              Prépare ton menu de coloc
            </h2>
            <p className="text-xs text-zinc-400">
              Calculé sur-mesure sans four, économique et plein de protéines.
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-zinc-800/60 grid grid-cols-2 gap-2 text-[11px] text-zinc-300">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>35g à 50g+ protéines / repas</span>
          </div>
        </div>
      </div>
      ...
      {/* 2. Repas par jour & Nombre de personnes */}
      <div className="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              className={`py-2 rounded-xl text-xs font-medium transition-all ${
                mealsPerDay === 2
                  ...
                  : 'bg-zinc-900/70 text-zinc-400 border border-zinc-800/60'
              }`}
            >
              2 (Midi/Soir)
            </button>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between bg-zinc-900/80 border border-zinc-800/80 rounded-xl px-2 py-1.5">
            <button
              type="button"
              disabled={numberOfPeople <= 1}
              onClick={() => setNumberOfPeople(p => Math.max(1, p - 1))}
              className="w-7 h-7 rounded-lg bg-zinc-800 text-zinc-300 flex items-center justify-center font-bold active:scale-90 transition-all disabled:opacity-30"
            >
              -
            </button>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-1 bg-zinc-900 px-2.5 py-1 rounded-lg border border-zinc-800">
        ...
        {/* Budget calculation indicator */}
        <div className="p-3 rounded-xl bg-zinc-900/70 border border-zinc-800/80 flex items-center justify-between text-xs">
          <div className="text-zinc-400">
            <span className="text-zinc-200 font-medium">{totalMeals} repas</span> au total ({numberOfPeople} pers.)
          </div>
          <div className="text-right">
            <span className="text-emerald-400 font-bold">{costPerMeal} €</span>{' '}
            <span className="text-[11px] text-zinc-400">/ portion</span>
          </div>
        </div>
      </div>

      {/* 5. Favoris intégrés notice */}
      ...
      >
        {isLoading ? (
          <>
            ...
            <span>Nourryr concocte ton programme...</span>
          </>
        ) : (
          <>
            <Sparkles className="w-4 h-4 fill-zinc-950 stroke-none" />
            <span>Générer le programme complet</span>
      ...
      {isLoading && (
        <div className="text-center p-3 text-xs text-zinc-400 animate-pulse">
          ⚡ Optimisation des protéines, vérification des prix chez {supermarket} et calcul sans aucun four...
        </div>
      )}
    </form>
  );
