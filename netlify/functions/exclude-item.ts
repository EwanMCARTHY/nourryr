...
Tes règles ABSOLUES :
1. APPORTS PROTÉINÉS TRÈS ÉLEVÉS : Chaque repas principal doit fournir STRICTEMENT entre 45g et 65g de protéines par portion (poulet, dinde, bœuf haché 5%, thon, œufs, skyr, etc.).
2. SATIÉTÉ MAXIMALE & ZÉRO SURPLUS CALORIQUE : Calories entre 600 et 750 kcal par portion, gros volume de légumes (200-300g), féculents rassasiants, 1 c. à café d'huile max.
3. ÉQUIPEMENT DISPONIBLE : STRICTEMENT plaques, poêle, casserole et micro-ondes. AUCUN FOUR.
4. RESPECT STRICT DU BUDGET & PREMIERS PRIX : Le coût total estimé du caddie ne doit JAMAIS dépasser le budget max. Utiliser les marques premiers prix (Eco+, Top Budget, Pouce) pour garantir le respect du budget.
5. SOBRIÉTÉ : Pas de blabla, pas de description verbeuse.
6. FORMAT DE RÉPONSE : Objet JSON valide conforme au schéma demandé.`;
...
3. ADAPTER LES RECETTES IMPACTÉES : Modifier les recettes concernées pour intégrer ce substitut, ajuster le titre si nécessaire, la liste des ingrédients et les étapes. Les recettes non impactées DOIVENT RESTER STRICTEMENT IDENTIQUES.
...
Réponds avec ce schéma JSON exact :
{
  ...
  "recipes": [
    ... liste complète des ${currentMealPlan.recipes.length} recettes (recettes non impactées à l'identique, recettes impactées adaptées avec 45-65g prot, 600-750 kcal, sans four)
  ],
  "updatedShoppingList": [
    {
  ...
  "estimatedTotalCost": nombre (STRICTEMENT <= ${currentMealPlan.budget})
    ...
    if (!parsed) {
      return {
        statusCode: 503,
        body: JSON.stringify({ error: `Modèles Gemini indisponibles (${lastErr}). Réessaie dans quelques secondes.` }),
      };
    }
